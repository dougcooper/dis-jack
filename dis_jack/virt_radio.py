...
class Address:
    ...
    def __str__(self) -> str:
        f'{self.site}:{self.host}:{self.entity}:{self.radio}'
...
class MySignalPdu(SignalPdu,Filter):
    ...
    def address(self):
        return Address(
            f'{self.entityID.siteID}:{self.entityID.applicationID}:{self.entityID.entityID}:{self.radioID}'
        )        
...
class VirtualRadio:
    ...
    async def _do_transmit(self):
        ...
        while True:
            ...
            while len(buffer) > self.pdu_sample_size_bytes:
                ...
                resampled_data,resample_state = audioop.ratecv(data,FLOAT_SIZE_BYTES,1,self.audio_samplerate,self.pdu_samplerate,resample_state)
                compressed_data = audioop.lin2ulaw(resampled_data,FLOAT_SIZE_BYTES)
    ...
    async def _do_receive(self):
        ...
        while True:
            ...
            pdu = createPdu(data)
            try:
                pdu = MySignalPdu.cast(pdu).filter(lambda pdu: str(pdu.address()) != self.address and pdu.encodingScheme == 1)
                if pdu is not None:
                    lin_data = audioop.ulaw2lin(pdu.data,FLOAT_SIZE_BYTES) #1 byte in assumed, 4 bytes out specified
                    resampled_data,resample_state = audioop.ratecv(lin_data,FLOAT_SIZE_BYTES,1,pdu.sampleRate,self.audio_samplerate,resample_state)
